...
import { PreRevealSting, isRevealStingActive } from './PreRevealSting';
...
export function Reveal({ room, clockOffsetMs = 0 }: { room: RoomApi; clockOffsetMs?: number }) {
  const state = room.state!;
  const round = state.currentRound!;
  const source = state.currentSource!;
  const [showOriginal, setShowOriginal] = useState(false);
  const resultsBtnRef = useRef<HTMLButtonElement | null>(null);
  // Re-render when the synced sting expires so the overlay unmounts without a
  // server broadcast (beat timing is client-derived from revealStingStartedAt).
  const [, setStingTick] = useState(0);

  const stingActive = isRevealStingActive(
    round.revealStingStartedAt,
    round.revealStingSkipped,
    clockOffsetMs,
  );

  useEffect(() => {
    if (!stingActive) return;
    const id = setInterval(() => setStingTick((n) => n + 1), 100);
    return () => clearInterval(id);
  }, [stingActive, round.revealStingStartedAt, round.revealStingSkipped]);

  const skipSting = useCallback(() => {
    if (room.isHost) room.skipRevealSting();
  }, [room]);
  ...
  useEffect(() => {
    if (stingActive || !room.isHost || !isLast || subs.length === 0) return;
    requestAnimationFrame(() => resultsBtnRef.current?.focus());
  }, [room.isHost, isLast, subs.length, stingActive]);
  ...
  }, [room, isLast, round.votingEnabled, subs.length, stingActive]);

  // ← / → / Space navigate the queue (host); hold O to peek at the original.
  useEffect(() => {
    if (stingActive || subs.length === 0) return;
  ...
  }, [room, idx, isLast, subs.length, stingActive]);
  ...
  if (subs.length === 0) {
    return (
      <>
        {stingActive && (
          <PreRevealSting
            stingStartedAt={round.revealStingStartedAt}
        ...
        <div className="card p-10 text-center animate-fade-up">
          ...
          {room.isHost && (
            <button className="btn-primary mt-5" onClick={room.showScoreboard} disabled={stingActive}>Continue →</button>
          )}
        </div>
      </>
    );
      ...
      <div className="flex flex-col md:grid md:grid-cols-[260px_1fr] gap-2 sm:gap-2.5 md:gap-3 animate-fade-up min-w-0 flex-1 min-h-0 h-full">
        ...
        <div className="flex flex-col gap-2 sm:gap-2.5 min-w-0 flex-1 min-h-0 md:h-full md:self-stretch">
          ...
          <div className="card flex-1 min-h-0 p-1.5 sm:p-2 flex flex-col gap-2">
            <div className="relative flex-1 min-h-0 w-full rounded-[2px] overflow-hidden bg-paper2 border-2 border-ink">
              ...
              <img
                ...
                className="absolute inset-0 w-full h-full object-contain animate-pop"
              />
              <img
                src={source.imageUrl}
                alt="Original source"
          ...
          <div className="shrink-0 flex flex-col sm:flex-row sm:items-end gap-2">
            ...
            {room.isHost ? (
              <div className={`flex items-center gap-2 shrink-0 ${subs.length > 1 ? 'sm:w-[min(100%,22rem)]' : 'w-full'}`}>
                <button className="btn-secondary" disabled={idx === 0 || stingActive} onClick={() => room.advanceReveal(-1)}>← Previous</button>
                {isLast ? (
                  <button
                    ref={resultsBtnRef}
                    className="btn-primary flex-1"
                    disabled={stingActive}
                    onClick={round.votingEnabled ? room.beginVoting : room.showScoreboard}
                  >
                    {round.votingEnabled ? 'Open ballot →' : 'See Results →'}
                  </button>
                ) : (
                  <button className="btn-primary flex-1" disabled={stingActive} onClick={() => room.advanceReveal(1)}>Next →</button>
                )}
              </div>
            ) : (
