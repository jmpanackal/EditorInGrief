...
import { AVATAR_PLATE_RADIUS, PlayerAvatar } from './PlayerAvatar';

/** Per-player Ready / Editing presence (edit phase). Rail uses avatar badges; list uses inline captions. */
export type PlayerPresenceStatus = 'ready' | 'editing';
...
export function PlayerList({
   ...
   * "how many slots are open" affordance). Pass room maxPlayers on lobby,
   * edit, reveal, and scoreboard rails/lists. */
  maxPlayers?: number;
  /** `rail` = compact horizontal avatar strip (mobile lobby); `list` = stacked rows. */
  layout?: 'list' | 'rail';
  /** Optional Ready / Editing presence from round submissions. */
  statusById?: Record<string, PlayerPresenceStatus>;
  /** Emphasize the player whose edit is currently on stage (reveal walkthrough). */
  highlightId?: string | null;
  /** When false (Reveal), hide empty-seat placeholders and only show filled seats. */
  showEmptySeats?: boolean;
}) {
  const sorted = showScores ? [...players].sort((a, b) => b.score - a.score) : players;
  const openSeats =
    showEmptySeats && maxPlayers != null ? Math.max(0, maxPlayers - players.length) : 0;
      ...
      >
          ...
          return (
            <li key={p.id} className="flex flex-col items-center gap-1 w-[3.75rem] shrink-0">
              <div className="relative">
                ...
                {status === 'ready' && (
                  <span
                    className="absolute -bottom-1 -left-1 w-4 h-4 rounded-full bg-grief text-paper border border-ink grid place-items-center text-[9px] font-black leading-none z-[1]"
                    title="Ready"
                    aria-label="Ready"
                  >
                    ✓
                ...
                {status === 'editing' && (
                  <span
                    className="absolute -bottom-1 -left-1 w-4 h-4 rounded-full bg-paper2 text-ink3 border border-ink grid place-items-center text-[9px] leading-none z-[1]"
                    title="Editing"
                    aria-label="Editing"
                  >
                    ✎
              ...
              <span
                className={`w-full text-center text-[11px] leading-tight truncate ${
                  highlighted
                    ? 'font-display font-black text-grief'
                    : p.connected
                      ? 'font-semibold text-ink'
                      : 'font-semibold text-ink3 line-through'
                }`}
              >
                {p.nickname}
              </span>
        ...
        {Array.from({ length: shownEmptySeats }).map((_, i) => (
          <li key={`empty-${i}`} className="flex flex-col items-center gap-1 w-[3.75rem] shrink-0 opacity-45">
            <div className={`w-11 h-11 ${AVATAR_PLATE_RADIUS} grid place-items-center border-2 border-dashed border-ink/40 text-ink3`}>
        ...
        {foldedEmptySeats > 0 && (
          <li className="flex flex-col items-center gap-1 w-[3.75rem] shrink-0 opacity-60">
            <div className={`w-11 h-11 ${AVATAR_PLATE_RADIUS} grid place-items-center border border-dashed border-ink/30`}>
    ...
    <ul className="flex flex-col divide-y divide-ink/15">
        ...
        return (
          ...
          >
            <div className="relative shrink-0">
              <PlayerAvatar nickname={p.nickname} avatarId={p.avatarId} connected={p.connected} size="md" />
              <span
                className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border border-ink ${
                  p.connected ? 'bg-grief' : 'bg-paper2'
                }`}
              />
              {status === 'ready' && (
                <span
                  className="absolute -bottom-1 -left-1 w-4 h-4 rounded-full bg-grief text-paper border border-ink grid place-items-center text-[9px] font-black leading-none"
                  title="Ready"
                  aria-label="Ready"
                >
                  ✓
            ...
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-1.5 gap-y-1">
              ...
              {showScores && i === 0 && p.score > 0 && (
                <span className="shrink-0" title="Front-page lead">
                  🏆
                </span>
              )}
              ...
              {status === 'ready' && (
                <span className="badge shrink-0 border-grief text-grief">Ready</span>
              )}
              {status === 'editing' && (
                <span className="text-ink3 italic text-sm shrink-0">Editing</span>
              )}
            </div>
      ...
      {foldedEmptySeats > 0 && (
        <li className="flex items-center gap-3 py-2 pl-2 border-l-2 !border-l-transparent opacity-70">
          ...
          <span className="text-ink3 italic text-sm">
            {foldedEmptySeats} more open seat{foldedEmptySeats === 1 ? '' : 's'}
          </span>
        </li>
      )}
    </ul>
